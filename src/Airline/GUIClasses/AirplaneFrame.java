...
public class AirplaneFrame extends JFrame {
	...
	public AirplaneFrame (Airplane airplane, Passenger passenger) {
		...
		departurePanel = new RouteInfoPanel(airplane.getRoute().getDeparture(), airplane.getRoute().getDepartTime());
		destinationPanel = new RouteInfoPanel(airplane.getRoute().getDestination(), airplane.getRoute().getLandingTime());
		seatPanel = new SeatPanel(airplane, passenger);
		minSize = new Dimension(500,500);
		spacing = new JPanel();
		spacing.add(new JLabel("\t"));
		
		this.airplaneName = new JLabel("Displaying information for airplane: " + airplane.getFlightNo());
		...
		this.add(this.departurePanel, BorderLayout.EAST);
		this.add(this.seatPanel, BorderLayout.CENTER);
		this.add(this.destinationPanel, BorderLayout.WEST);
